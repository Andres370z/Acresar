...
@Component({
  selector: 'app-ajuste-facultativo',
  templateUrl: './ajuste-facultativo.component.html',
  styleUrls: ['./ajuste-facultativo.component.css']
})
export class AjusteFacultativoComponent implements OnInit {
  ...
  procesar() {
    ...
    //   this.alertService.info('Hey','Campo ramos es obligatorio'); llega undefined
    // }
    // tslint:disable-next-line:one-line
  ...
  guardareditarramos() {
    ...
    // tslint:disable-next-line:one-line
      ...
      this.http.putFaculUpdate(this.final.a, data).then(
        res => {
          this.loadramos()
          this.alertService.success('Ok', 'editaste...')
        },
        err => {
          console.log(err);
        });
