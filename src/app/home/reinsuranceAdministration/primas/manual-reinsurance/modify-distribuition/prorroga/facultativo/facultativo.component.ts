...
export class FacultativoComponent implements OnInit {
  ...
  cargar(item) {
    ...
    this.http.getFacultativoContra('/facultativo/contrato/' + item.pro_id).then(
      res => {
        this.listareasu = res;
        console.log(res);
        this.alerthService.messagefin();
  ...
  procesar() {
    ...
    // tslint:disable-next-line:one-line
    // else if (this.selectcontrato.length === 0) { ESTA LLEGANDO UNDEFINED
    //   this.alerthService.info('Hey','Campo ramos es obligatorio');
    // }
    // tslint:disable-next-line:one-line
  ...
  cerrar() {
    ...
    this.contratofinal = [];
    this.listareasu = [];
    this.router.navigate(['home/reinsuranceAdministration/primas/modify-distribuition/prorroga']);
  }
  verdetalle(item: any) {
