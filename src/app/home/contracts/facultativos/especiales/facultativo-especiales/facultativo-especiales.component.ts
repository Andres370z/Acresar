...
export class FacultativoEspecialesComponent implements OnInit {
  ...
  modulo: string = 'Facultativo Especial';
  selectMoneda: any;
  sumaComision = 0;
  ...
  create() {
    if (!sessionStorage.getItem('formCuotaP')) {
      ...
      // tslint:disable-next-line:one-line
        ...
        const data1 = {
          codigocontrato: 'ATL-FAC-ESP-' + this.cod,
        };
        this.alertService.loading();
        ...
        /*  this.service.postQueryrespaldo(data1, '/contratos/facultativo/buscar/contrato').then(
           res => {
             const buscar = res
             console.log(buscar)
             if (buscar === "N.N") {
               ...
               this.service.postQueryrespaldo(data, '/facultativo/contrato').then(
                 res => {
                   localStorage.setItem('idcontrato', JSON.stringify(res));
                   this.contrato = JSON.parse(localStorage.getItem('idcontrato'));
                   this.statefinal = true;
  ...
  verificar() {
    if (sessionStorage.getItem('formCuotaP') && sessionStorage.getItem('formCuotaP1')) {
      ...
      this.alertService.messageInfo('Hey','Desea salir sin agregar ramos al contrato');
    }
  }
